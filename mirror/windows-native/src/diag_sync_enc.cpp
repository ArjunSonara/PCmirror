...
int main() {
    ...
    printf("Found %u synchronous H.264 encoders.\n", count);
    if (count == 0) return 1;

    WCHAR name[256] = {};
    activates[0]->GetString(MFT_FRIENDLY_NAME_Attribute, name, 255, nullptr);
    wprintf(L"Selected sync encoder: %s\n", name);

    ComPtr<IMFTransform> encoder;
    HRESULT hr = activates[0]->ActivateObject(IID_PPV_ARGS(encoder.GetAddressOf()));
    ...
    if (SUCCEEDED(encoder.As(&codecApi))) {
        ...
        printf("Configured low-latency CodecAPI!\n");
    }
    ...
    printf("OutputStreamInfo: cbSize=%u, dwFlags=0x%08x\n", sInfo.cbSize, sInfo.dwFlags);
    ...
    for (int f = 0; f < 10; f++) {
        ...
        for (;;) {
            ...
            if (FAILED(hr)) { printf("  ProcessOutput hr=0x%08x\n", hr); break; }

            DWORD len = 0;
            outBuf.pSample->GetTotalLength(&len);
            printf("  >>> ENCODED OUTPUT FRAME READY! Length: %u bytes\n", len);
        }
    }
