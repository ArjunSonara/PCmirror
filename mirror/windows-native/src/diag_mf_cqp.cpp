...
int main() {
    ...
    if (SUCCEEDED(encoder.As(&codecApi))) {
        ...
        // Rate control: Quality (CQP)
        v.vt = VT_UI4; v.ulVal = eAVEncCommonRateControlMode_Quality;
        codecApi->SetValue(&CODECAPI_AVEncCommonRateControlMode, &v);
        // Quality level
        v.vt = VT_UI4; v.ulVal = 70;
        codecApi->SetValue(&CODECAPI_AVEncCommonQuality, &v);
        // Worker threads = 1 (disables slice/frame threading buffer)
        v.vt = VT_UI4; v.ulVal = 1;
        codecApi->SetValue(&CODECAPI_AVEncNumWorkerThreads, &v);
        // Buffer window = 0 or 1
        v.vt = VT_UI4; v.ulVal = 1;
    ...
    for (int f = 0; f < 20; f++) {
        ...
        for (;;) {
            ...
            printf("Input %2d -> Output %2d: %u bytes\n", f, totalOutputs, len);
            totalOutputs++;
        }
    }
